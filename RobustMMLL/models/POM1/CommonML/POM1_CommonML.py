...
class POM1_CommonML_Master(POM1ML):
    ...
    def CheckNewPacket_Master(self):
        ...
        if self.platform == 'pycloudmessenger':
            ...
            sender = None
            try:
                packet = self.comms.receive_poms_123(timeout=0.3) # We only receive a dictionary at a time even if there are more than 1 workers
...
class POM1_CommonML_Worker(POM1ML):
    ...
    def CheckNewPacket_worker(self):
        ...
        if self.platform == 'pycloudmessenger':
            ...
            sender = None
            try:
                packet = self.comms.receive_poms_123(timeout=0.3)
                packet = packet.content
                sender = 'Master'
